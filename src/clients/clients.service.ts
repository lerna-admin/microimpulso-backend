...
export class ClientsService {
  ...
  async findAll(
    ...
    page:  number = 1,
    filters?: {
      status?:      'active' | 'inactive' | 'rejected' | 'prospect'; // ← added “prospect”
      document?:    string;
      name?:        string;
      mode?:        string;
      type?:        string;
    ...
    /* ------------------------------------------------------------------ */
    /* 1. Load loan requests as before                                    */
    /* ------------------------------------------------------------------ */
    ...
    for (const c of allClients) {
      if (!clientMap.has(c.id)) clientMap.set(c.id, []); // empty array → prospect
    }
    
    /* ------------------------------------------------------------------ */
    /* 4. Metrics and result builder                                      */
    /* ------------------------------------------------------------------ */
    ...
    /* ------------------------------------------------------------------ */
    for (const [clientId, clientLoans] of clientMap) {
      const client =
      clientLoans.length > 0
      ? clientLoans[0].client
      : allClients.find(c => c.id === clientId)!; // prospect
      ...
      /* Prospect: lightweight push and continue */
      if (status === 'prospect') {
        allResults.push({
          client,
          loanRequest:       null,
